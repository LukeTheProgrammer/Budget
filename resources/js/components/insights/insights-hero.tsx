                ...
                <p className="mt-2 max-w-xs text-sm opacity-90">
                    {isOver ? (
                        <>
                            ...
                            <b>
                                {formatMoney(
                                    summary.total_overage_cents,
                                    currency,
                                )}
                            </b>{' '}
                            more than your combined caps allowed across{' '}
                            {summary.categories_over} of{' '}
                            {summary.categories_total} categories.
                        </>
                    ) : (
            ...
            <div className="grid grid-cols-2 divide-x divide-y border-l bg-card">
                ...
                <Stat
                    ...
                    value={
                        <span
                            className={cn(
                                summary.vs_previous_cents > 0 &&
                                    'text-destructive',
                            )}
                        >
                            {formatSignedMoney(
                                summary.vs_previous_cents,
                                currency,
                            )}
                        </span>
                    }
                ...
                <Stat
                    ...
                    value={
                        <span className="text-2xl">
                            {summary.biggest_contributor?.name ?? '—'}
                        </span>
                    }
                    note={
                ...
                <Stat
                    ...
                    value={
                        summary.largest_charge
                            ? formatMoney(
                                  summary.largest_charge.amount_cents,
                                  currency,
                              )
                            : '—'
                    }
